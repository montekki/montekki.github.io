...
https://docs.rs/ark-poly/0.3.0/ark_poly/polynomial/multivariate/struct.SparsePolynomial.html
...
https://docs.rs/ark-poly/0.3.0/ark_poly/struct.DenseMultilinearExtension.html
...
Next, two trivial methods that just forward to corresponding methods
in the `DenseMultilinearExtension`:
...
single-variate `DenseMultilinearExtension` over variable $i$.

Now the only thing separating us from the finalizing implementation
of the needed trait is turning this single-variate extension into a
single-variate `SparsePolynomial`, since, you know, we are using Rust
to typesafely guarantee that the polynomial is actually single-variate.

What we know is that $g$ is a polynomial with degree at most $2$ in
each variable so this single-variate extension is an extension polynomial
...
https://docs.rs/ark-poly/0.3.0/ark_poly/domain/general/enum.GeneralEvaluationDomain.html
)
and [`Evaluations`](
https://docs.rs/ark-poly/0.3.0/ark_poly/evaluations/univariate/struct.Evaluations.html
...
The Book talks about. However recall that _Premature optimization is the root of all evil_ and we were more focusing here on generalizing the protocol
